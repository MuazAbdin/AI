...
MAX_ITERATIONS = 10     # at least 1000
EXPANSION_THRESHOLD = 350  # branching factor ~ 700 : expand half of it
...
def UCT(initial_state):
    ...
    for i in range(MAX_ITERATIONS):
        ...
        while (len(node.remaining_moves) <= max(fully_unexpanded - EXPANSION_THRESHOLD, 0)) and \
                node.child_nodes:
            print(f' len -> {len(node.remaining_moves)}', end=', ')

            node = node.select_child()
            print(f" select {node.initiating_move}", end=', ')
