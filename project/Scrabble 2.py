...
class ScrabbleApp:
    ...
    # def _display_winner(self, parent):
    #     winner_val = tk.StringVar()
    #     winner_val.set('')
    #     winer_frame = tk.Frame(parent, height=2, bd=4, relief=tk.SUNKEN)
    #     # winer_frame.pack(pady=(0, 10), padx=10)
    #     winer_frame.place(relx=0.15, rely=0.45)
    #     winner = tk.Label(winer_frame, height=2, width=10, font=ScrabbleApp.FONT_3,
    #                       textvariable=winner_val,
    #                       fg=ScrabbleApp.SCORE_TIME_FG, bg=ScrabbleApp.SUB_IN_BG)
    #     winner.pack()
    #     return winner_val
    ...
    def _create_game(self, option: str):
        ...
        self._score_widget(game)

        self._board = self._init_cubes_board(game)
        self._display_rack(game, '', option)
    ...
    def _one_play_button(self, parent):
        start_frame = tk.Frame(parent, height=75, width=100)
        start_frame.pack_propagate(False)
        start_frame.place(relx=0.2, rely=0.65)
        return start_frame
    ...
    def _create_mcts_game(self):
        ...
        def _play_mcts(event, parent):
            ...
            if widget == parent and self._current_player != 'MCTS':
                ...
                # mcts_board = STANDARD
                # mcts_board = Board(BOARD, [Player('p1', highest_scoring_play)])
                self.play_game(mcts_board, board_frame, rack_frame, scores_StringVar)
        ...
        def _play_one_mcts(event, parent):
            ...
            if widget == parent:
                ...
                # mcts_board = STANDARD
                # mcts_board = Board(BOARD, [Player('p1', highest_scoring_play)])
                self._one_round(mcts_board, board_frame, rack_frame, scores_StringVar)
    ...
    def _one_round(self, board, board_frame, rack_frame, scores_StringVar):
        board = board.make_one_play(board.players[0], verbose=True)
