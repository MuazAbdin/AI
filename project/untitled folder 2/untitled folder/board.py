...
class Board(list):
    ...
    def make_one_play(self, play, verbose=False):
        ...
        # print(f'play.letters = {play.letters}, p.num_exchanges = {p.num_exchanges}, '
        #       f'self.all_exchanges {self.all_exchanges}')
...
if __name__ == '__main__':
    ...
    # play_game(board=Board(BOARD, [Player('Greedy', AI.greedy)]))
    play_game(board=Board(BOARD, [Player('UCT', AI.UCT)]))

    # board = Board(BOARD, [Player('Greedy', AI.greedy)])
    # # print(len(board.bag))
    # p = board.players[0]
    # # print(p.rack)
    # p.rack = exchange_letters(p.rack, board.bag)
    # # print(p.rack)
    # # print(len(board.bag))

    # for i in range(50):
    #     play_game(board=Board(BOARD, [Player('Greedy', AI.greedy)]), verbose=False)
