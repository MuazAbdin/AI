...
MAX_ITERATIONS = 300     # at least 1000
...
def UCT(initial_state):
    ...
    # fully_unexpanded = list(state.all_plays())
    #
    # tail = max(-len(fully_unexpanded), -TAIL_IDX)
    # chosen_plays = sorted(fully_unexpanded, key=lambda play: state.score(play))[tail:]
    # print(f'len(chosen_children) = {len(chosen_plays)}')
    #
    # root_node.remaining_moves = chosen_plays
    #
    # for play in chosen_plays:
