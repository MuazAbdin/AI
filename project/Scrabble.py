...
class ScrabbleApp:
    ...
    TILES_IMGs = {'A': './letters/A.png', 'B': './letters/B.png', 'C': './letters/C.png',
                  'E': './letters/E.png', 'F': './letters/F.png', 'G': './letters/G.png',
                  'H': './letters/H.png', 'I': './letters/I.png', 'J': './letters/J.png',
                  'K': './letters/K.png', 'L': './letters/L.png', 'M': './letters/M.png',
                  'N': './letters/N.png', 'O': './letters/O.png', 'P': './letters/P.png',
                  'Q': './letters/Q.png', 'R': './letters/R.png', 'S': './letters/S.png',
                  'T': './letters/T.png', 'U': './letters/U.png', 'V': './letters/V.png',
                  'W': './letters/W.png', 'X': './letters/X.png', 'Y': './letters/Y.png',
                  'Z': './letters/Z.png', '_BLANK': './letters/_BLANK.png'}
    ...
    def _create_options(self, parent):
        ...
        # mcts_button = self._create_button(parent, self._quit_game, text='Monte Carlo Tree Search')
        # mcts_button.place(relwidth=0.3, relheight=0.1, relx=0.65, rely=0.2)
    ...
    def _create_pdf_video_html_buttons(self, parent):
        ...
        pdf_button = self._create_button(pdf_frame, self._quit_game, self._pdf_image)
        ...
        html_button = self._create_button(html_frame, self._quit_game, self._html_image)
        ...
        video_button = self._create_button(video_frame, self._quit_game, self._video_image)
        ...
        about_button = self._create_button(about_frame, self._quit_game, self._about_image)
    ...
    def _to_menu(self, event, parent):
        ...
        if widget == parent:
            for game in ['MCTS', 'MiniMax', 'MCTS vs MiniMax']:
                if game in self._game_frames.keys():
                    self._game_frames[game].destroy()
            self._raise_frame(self._game_frames['main_menu'])
    ...
    def _quit_game(self, event, parent):
        ...
        if widget == parent:
            if tk.messagebox.askokcancel(ScrabbleApp.EXIT_TITLE,
    ...
    def _score_widget(self, parent):
        ...
        self._display_score(p1_score_frame, 'MCTS')
        ...
        self._display_score(p2_score_frame, '')

    def _display_score(self, parent, player: str):
        self._score_val = tk.StringVar()
        self._score_val.set(str(self._score))
    ...
    def _create_mcts_game(self):
        ...
        self._score_widget(mcts_game)

        board_frame = self._init_cubes_board(mcts_game)
        self._display_rack(mcts_game, 'AM_C', 'MCTS')

        self._init_cube(board_frame, (7, 6), image=self._tiles_imgs['_'], text='m')
        self._init_cube(board_frame, (7, 7), image=self._tiles_imgs['U'])
        self._init_cube(board_frame, (7, 8), image=self._tiles_imgs['A'])
        self._init_cube(board_frame, (7, 9), image=self._tiles_imgs['Z'])

        self._init_cube(board_frame, (8, 8), image=self._tiles_imgs['L'])
        self._init_cube(board_frame, (9, 8), image=self._tiles_imgs['O'])
        self._init_cube(board_frame, (10, 8), image=self._tiles_imgs['N'])
