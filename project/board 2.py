...
class Board(list):
    ...
    """
    # average score of single greedy player
    SINGLE_AVG_SCORE = 850
    ...
    def clone(self):
        ...
        #     cur_p_idx = self.players.index(self.current_player)
        #     copy.current_player = self.players[cur_p_idx]
        #     # copy.current_player = self.current_player.clone()
        return copy
    ...
    def make_one_play(self, play, verbose=False):
        ...
        """
        # print(f'ID = {hex(id(self))}')
        # if verbose:
        # print(play)
        # print(self)
        # p = self.current_player
        p = self.players[self.cur_p_idx]
        # p_idx = self.players.index(self.current_player)
        rack = self.current_player.rack

        self.current_player.rack = replenish(play.rack, self.bag)
        ...
        if self.current_player.rack == '':
            scores = subtract_remaining_tiles(racks, scores, self.current_player)

        # cur_idx = self.players.index(self.current_player)
        self.current_player = self.players[(p_idx + 1) % len(self.players)]

        if verbose:
            print('Player {} with rack {} makes {} for {} points; draws: {}; scores: {}'
                  .format(self.current_player.name, rack, play, points, self.current_player.rack,
                          scores))
            print(self, end='\n')
    ...
    def get_result(self, player):
        if len(self.players) == 1:
            return 1.0 if player.score >= (Board.SINGLE_AVG_SCORE*2/3) else 0.0
        return 1.0 if player.score == max([p.score for p in self.players]) else 0.0

    def heavy_playout(self):
        # score of the played word
        m1 = max(self.all_plays(), key=lambda play: self.score(play))
...
def play_game(board=None, verbose=True):
    ...
    while True:
        ...
        for p in board.players:
            ...
            print(f'board id: {hex(id(board))}\n'
                  f'player: {board.current_player.name}, racks: {racks}, scores: {scores}.\n')
...
if __name__ == '__main__':
    ...
    # play_game()
    play_game(board=Board(BOARD, [Player('UCT', AI.UCT)]))

    # Ngames = 20
    # results = [play_game(verbose=False) for _ in range(Ngames)]
