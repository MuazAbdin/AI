...
def blokus_corners_heuristic(state, problem):
    ...
    # return (cost + len(vacant_corners)) // 2

    # for corner in vacant_corners:
    #     # min_dist = max(state.board_w, state.board_h)
    #     min_dist = 0
    #     for tile in occupied_tiles:
    #         min_dist = abs(min_dist - distance(corner, tile))
    #         # min_dist = min(min_dist, distance(corner, tile))
    #         # min_dist = min(min_dist, euclideanDistance(corner, tile))
    #         # min_dist = min(min_dist, chebyshevDistance(corner, tile))
    #         # manhattanDistance is better, but is it admissible for our problem?
    #         # min_dist = min(min_dist, util.manhattanDistance(corner, tile))
    #     # cost = min(cost, min_dist)
    #     cost += min_dist
    #
    # return cost
...
class ClosestLocationSearch:
    ...
    def solve(self):
        ...
        while targets:
            # closest_target = get_closest_target(current_state, targets)
