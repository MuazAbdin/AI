...
# we created this class in order to be able to push to PriorityQueue
# the state itself, the the actions leading to it, and the cost.
class Node:
...
def uniform_cost_search(problem):
    ...
    start_state = problem.get_start_state()
    fringe.push(Node(start_state, [], 0), 0)
    visited = set()
    ...
    while not fringe.isEmpty():
        ...
        if current.state not in visited:
            ...
            for state, action, cost in problem.get_successors(current.state):
                if state not in visited:
                    new_cost = current.cost + cost
                    new_actions = current.actions + [action]
                    fringe.push(Node(state, new_actions, new_cost), new_cost)
    return list()
