...
class ReflexAgent(Agent):
    ...
    def evaluation_function(self, current_game_state, action):
        ...
        values = np.array([score, empty_tiles, cluster, monotonicity, uniformity])
        weights = np.array([1.5, 1, -1, 1, 1])
    ...
    def _alpha_beta(self, state, depth, alpha, beta, ply):
        ...
        if ply == self.AGENT:
            ...
            for action in legal_actions:
                ...
                opponent_score = self._alpha_beta(opponent, depth - 1, alpha, beta, self.OPPONENT)[
                    1]
...
def better_evaluation_function(current_game_state):
    ...
    empty_tiles = current_game_state.get_empty_tiles()[0].size
