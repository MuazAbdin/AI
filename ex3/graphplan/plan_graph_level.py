...
class PlanGraphLevel(object):
            ...
            if previous_proposition_layer.all_preconds_in_layer(action):
                ...
                for pre1 in action_pre:
                    for pre2 in action_pre:
                        if (pre1 != pre2) and previous_proposition_layer.is_mutex(pre1, pre2):
                            update = False
                if update:
                    self.action_layer.add_action(action)
        # ------------------------------------------------------------------ #

    def update_mutex_actions(self, previous_layer_mutex_proposition) -> None:
        """
    ...
    def update_proposition_layer(self) -> None:
        ...
        for action in current_layer_actions:
            for prop in action.get_add():
                name = prop.get_name()
                if name not in added_propositions:
                    # make an instance of Proposition
                    added_propositions[name] = Proposition(name)
                # update the producers list
                added_propositions[name].add_producer(action)
        # updates the propositions in the current proposition layer
        for prop in added_propositions.values():
    ...
    def expand(self, previous_layer):
        ...
        "*** YOUR CODE HERE ***"
        # I. set the actions and their mutex relation in the action layer
        self.update_action_layer(previous_proposition_layer)
        self.update_mutex_actions(previous_layer_mutex_proposition)
        # II. set the propositions and their mutex relation in the propositions layer
...
def have_competing_needs(a1: Action, a2: Action, mutex_props) -> bool:
    """
    ...
    """
    "*** YOUR CODE HERE ***"
    # competing needs : the actions have preconditions that are mutually
    # exclusive at the proposition layer of the previous level.
    for p in a1.get_pre():
        for q in a2.get_pre():
...
def mutex_propositions(prop1, prop2, mutex_actions_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    # inconsistent support: two propositions are mutex, if all ways of achieving the
    # propositions (that is, actions at the same level) are pairwise mutex.
