    ...
    def get_successors(self, state):
        ...
        for action in self.actions:
            if (not action.is_noop()) and action.all_preconds_in_list(state):
                succ = set(prop for prop in state)
                for prop in action.get_add():
                    succ.add(prop)
                for prop in action.get_delete():
                    succ.discard(prop)
                successors.append((frozenset(succ), action, 1))
        return successors
...
def max_level(state, planning_problem):
    ...
    level_count = 0
    cur_level = pg_init
    # cur_level_props = pg_init.get_proposition_layer().get_propositions()
    cur_level_props = state
    while not planning_problem.is_goal_state(cur_level_props):
        if is_fixed(graph, level_count):
            return float('inf')
        level_count += 1
        next_level = PlanGraphLevel()
        next_level.expand_without_mutex(cur_level)
        graph.append(next_level)
        cur_level_props = next_level.get_proposition_layer().get_propositions()
...
def level_sum(state, planning_problem):
    ...
    while not planning_problem.is_goal_state(cur_level_props):
        if is_fixed(graph, level_count):
            return float('inf')
        for goal in list(left_goals):
            if goal in cur_level_props:
                level_costs_sum += level_count
